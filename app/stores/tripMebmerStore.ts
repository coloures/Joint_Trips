...
export const useTripStore = defineStore('trip', () => {
  ...
  const getTripMembersByTripId = (trip_id: number): TripMember[] | null => {
    return trip_members.value.filter(t => t.trip_id === trip_id) || null
  }

  const getTripMembersByMemberId = (member_id: number): TripMember[] | null => {
    return trip_members.value.filter(t => t.member_id === member_id) || null
  }
